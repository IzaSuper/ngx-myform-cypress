...
Cypress.Commands.add('checkDisability', () => {
  cy.get('#hotels').should('have.value', null)
  cy.get('#message').should('contain.text', 'Please choose a hotel from the menu')
  cy.get('#total').should('be.disabled')
  cy.get('#check').should('have.class', 'disabled')
})
Cypress.Commands.add('typeCheckInDate', () => {
  const currentDate = new Date()
  currentDate.setDate(currentDate.getDate() + 1)
  const checkIn1 = currentDate.toISOString().split('T')[0]
  cy.get('#from').type(checkIn1)
})

Cypress.Commands.add('typeCheckOutDate', () => {
  const currentDate = new Date()
  currentDate.setDate(currentDate.getDate() + 3)
  const checkOut1 = currentDate.toISOString().split('T')[0]
  cy.get('#to').type(checkOut1)
})
